...
exports.run = async (client, message, args) => {
  ...
  if (opponent.bot) return message.reply('<a:carpi:772430970896908298>  Botlar ile oynayamazsın!');
  if (opponent.id === message.author.id) return message.reply(':hayir:Kendin ile düello Atamazsın');
		if (this.fighting.has(message.channel.id)) return message.reply(':hayir: Kanal başına sadece bir düello meydana gelebilir.');
		...
		try {
			if (!opponent.bot) {
				...
				if (!verification) {
					...
					return message.channel.send(`<a:cokuzgun:777299839788253214> Düello kabul edilmedi...`);
				}
			}
			let userHP = 500;
			let oppoHP = 500;
			...
			while (userHP > 0 && oppoHP > 0) { // eslint-disable-line no-unmodified-loop-condition
				...
				if (!opponent.bot || (opponent.bot && userTurn)) {
					...
					if (!turn.size) {
						await message.reply(`<a:carpi:772430970896908298> Üzgünüm ama, süre doldu!`);
						reset();
						continue;
					}
				...
				if (choice === 'saldır') {
					const damage = Math.floor(Math.random() * (guard ? 10 : 60)) + 1;
					...
					if (!miss) {
						const damage = randomRange(100, guard ? 130 : 290);
					...
					await message.reply('<a:carpi:772430970896908298> Ne yapmak istediğini anlamadım.');
				}
			}
			this.fighting.delete(message.channel.id);
